      ...
      <div class="blog-featured-card">
        ...
        <div class="blog-featured-body">
          <div class="blog-featured-meta">
            ...
            <span class="blog-featured-cat">{{ category }}</span>
            {% endfor %}
          </div>
    ...
    <div class="column is-4">
      ...
      <div class="box">
        ...
        <div class="blog-archives">
          ...
          <details class="blog-archive-year">
            <summary>
              <span class="archive-year-label">{{ yearGroup.name }}</span>
              <span class="tag is-light">{{ yearGroup.items.size }}</span>
            </summary>
            <ul class="archive-posts-list">
              {% for post in yearGroup.items %}
              <li>
                <span class="archive-date">{{ post.date | date: "%d %b" }}</span>
                <a href="{{ post.url }}">{{ post.title | truncate: 45 }}</a>
              </li>
              {% endfor %}
            </ul>
          </details>
...
.blog-hero-bg::after {
  ...
  position: absolute;
  inset: 0;
  background: radial-gradient(ellipse at 60% 40%, rgba(100,100,255,0.18) 0%, transparent 70%),
              radial-gradient(ellipse at 20% 80%, rgba(0,200,255,0.12) 0%, transparent 60%);
}
...
.blog-pills-row::-webkit-scrollbar { display: none; }
.blog-pill {
  display: inline-flex;
  align-items: center;
  gap: 0.35rem;
...
.blog-featured-image-overlay {
  position: absolute;
  inset: 0;
  background: linear-gradient(to bottom, transparent 40%, rgba(15,20,60,0.95) 100%);
}
.blog-featured-body {
...
.blog-archive-year summary:hover { background: #e9ecef; }
.archive-year-label { color: #363636; }
.archive-posts-list {
  list-style: none;
  margin: 0;
  padding: 0.5rem 0;
...
@media screen and (max-width: 768px) {
  .blog-hero { min-height: 280px; margin: -0.5rem -0.5rem 1.5rem; }
...
/* ===========================
   MISC UTILITIES
=========================== */
.search-results-info { margin-bottom: 1rem; }
.category-list { display: flex; flex-direction: column; gap: 0.5rem; }
.category-item { border-radius: 6px; transition: background-color 0.2s; }
...
.empty-state { background-color: #fafafa; border-radius: 6px; padding: 2rem; }
.posts-grid {
  display: grid;
